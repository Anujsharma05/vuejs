<template>
  <div id="forms">
    ...
    <hr>
    <div>
      <input type="checkbox" id="apple" v-model="fruits" value="apple"><label for="apple">Apple</label>
    <br>
    <input type="checkbox" id="mango" v-model="fruits" value="mango"><label for="mango">Mango</label>
    {{ fruits }}
    </div>
    <hr>
    <div>
    ...
    <div>
      <select v-model="choose" multiple>
        ...
        <option v-for="(x, index) in selectOptions" :key="index" :selected="x=='C'">{{ x }}</option>
      </select>
      <br> 
      {{ choose }}
    </div>
...
export default {
  ...
  computed: {
    printPassword: function() {
      return 'this is the password:' + this.userData.password;
    }
  },
  methods: {
    submitted: function() {
